...
pub fn sep_list_common<'a,Pe,Re,Ps,Rs>(elem:Pe,sep:Ps) -> impl Parser<'a,u8,Vec<Re>>+Clone
where
    Pe:  Parser<'a,u8,Re>+Clone,
    Ps:  Parser<'a,u8,Rs>+Clone,
{   
		let space = take(seq(is_space,SeqCount::None));
	  sep_list( 
	  	between_opt(space.clone(),elem.clone(),space.clone()),
			sep,
			left(right_opt(space.clone(),elem), alt((space, data_end))),
		)
}
